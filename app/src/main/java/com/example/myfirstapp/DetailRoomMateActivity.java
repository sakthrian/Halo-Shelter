...
public class DetailRoomMateActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_detail_room_mate);

        name=findViewById(R.id.detail_roommate_name);
        rent=findViewById(R.id.detail_roommate_rent);
        location=findViewById(R.id.detail_roommate_location);
        gender=findViewById(R.id.detail_roommate_gender);
        occupancy=findViewById(R.id.detail_roommate_occupancy);
        looking_for=findViewById(R.id.detail_roommate_lookingfor);
        description=findViewById(R.id.detail_roommate_description);
        profileImage=findViewById(R.id.detail_roommate_profile_img);
        contact=findViewById(R.id.detail_roommate_contact);
        teams=findViewById(R.id.detail_roommate_team);
        make_team_btn=findViewById(R.id.detail_roommate_make_team_btn);
        decline_req_btn=findViewById(R.id.detail_roommate_decline_team_btn);
        ...
        if(bundle!=null){
           ...
           // occupancy.setText(occupancys);
            ...
            if(team.equals("Yes")){
                teams.setText("Interested in making teams");
            }else
            {
                teams.setText("Not Interested in making teams");
                ...
                public void onComplete(@NonNull Task<Void> task) {
                    if(task.isSuccessful()){
                        friendRef.child(uid).child(user.getUid()).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                            ...
                            public void onComplete(@NonNull Task<Void> task) {
                                if(task.isSuccessful()){
                                    Toast.makeText(DetailRoomMateActivity.this, "teammate removed", Toast.LENGTH_SHORT).show();
                ...
                public void onComplete(@NonNull Task<Void> task) {
                    if(task.isSuccessful()){
                        requestRef.child(uid).child(user.getUid()).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                            ...
                            public void onComplete(@NonNull Task<Void> task) {
                                if(task.isSuccessful()){
                                    Toast.makeText(DetailRoomMateActivity.this, "team request declined", Toast.LENGTH_SHORT).show();
                                    CurrentState="nothing_happen";
                                    make_team_btn.setText("make team");
                                    decline_req_btn.setVisibility(View.GONE);
                                }
    ...
    private void PerformAction() {
        if(CurrentState.equals("nothing_happen")){
            ...
            requestRef.child(user.getUid()).child(uid).updateChildren(hashMap).addOnCompleteListener(new OnCompleteListener() {
                ...
                public void onComplete(@NonNull Task task) {
                    if(task.isSuccessful()){
                        Toast.makeText(DetailRoomMateActivity.this, "Team request has sent", Toast.LENGTH_SHORT).show();
                        decline_req_btn.setVisibility(View.GONE);
                        CurrentState="I_sent_pending";
                        make_team_btn.setText("cancel team request");
                    }else{
                        Toast.makeText(DetailRoomMateActivity.this, task.getException().toString(), Toast.LENGTH_SHORT).show();
                    }
                }
            });
        }
        if(CurrentState.equals("I_sent_pending")||CurrentState.equals("I_sent_decline")){
            requestRef.child(user.getUid()).child(uid).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                ...
                public void onComplete(@NonNull Task<Void> task) {
                    if(task.isSuccessful()){
                        Toast.makeText(DetailRoomMateActivity.this, "Team request cancelled", Toast.LENGTH_SHORT).show();
                        CurrentState="nothing_happen";
                        make_team_btn.setText("make team");
                        decline_req_btn.setVisibility(View.GONE);
                    }else{
                            ...
                            public void onComplete(@NonNull Task task) {
                                if(task.isSuccessful()){
                                    friendRef.child(uid).child(user.getUid()).updateChildren(myhashMap).addOnCompleteListener(new OnCompleteListener() {
                                        @Override
                                        public void onComplete(@NonNull Task task) {
                                            Toast.makeText(DetailRoomMateActivity.this, "new teammate added", Toast.LENGTH_SHORT).show();
                                            CurrentState="friend";
                                            make_team_btn.setText("chat");
                                            decline_req_btn.setText("remove team");
                                            decline_req_btn.setVisibility(View.VISIBLE);
